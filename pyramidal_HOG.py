...
for img in glob.glob("/home/akash/Documents/Transpack/Datasets/training/*.jpg"):
    ...
    path.append(img)
    labels_full.append(img[51:])
features_list=[]
f=open("pyramidal_training_features_label.txt","w+")    
...
# Histogram of the first cell in the first block.
    ...
    for length in range(4,65,4):
        for i in range(length,129,4):
            for j in range(length,65,4):
                cell_direction = grad_direction[i-length:i, j-length:j]
                cell_magnitude = grad_magnitude[i-length:i, j-length:j]
                HOG_cell_hist=HOG.HOG_cell_histogram(cell_direction, cell_magnitude, hist_bins)
            # print(HOG_cell_hist)
                feature=np.append(feature,HOG_cell_hist/math.pow(2,(length/8)-1))
                print(feature)
            # print(feature)
...
# np.savetxt("pyramidal_training_features.txt",features_list)
hist_correct=[]
hist_incorrect=[]
correct=0
incorrect=0
...
for i in range(len(images)):
    for j in range(i+1,len(images)):
        ...
        if labels[i]==labels[j]:
            ...
            total_incorrect+=1
plt.figure(1)
plt.hist(hist_correct,1000,[0.5,1])
plt.title("same_test")
plt.figure(2)
plt.hist(hist_incorrect,1000,[0.5,1])
